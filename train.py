...
# print(data[data['transmission'] == 'Sedan'].head(3))
# # صف سليم (بافتراض إن transmission فيه Automatic/Manual)
# print(data[data['transmission'] == 'Automatic'].iloc[0])

# print("---")

# # صف مصاب
# print(data[data['transmission'] == 'Sedan'].iloc[0])
# print(len(data[data['transmission'] == 'Sedan']))
# # احفظ نسخة قبل الحذف (احتياط)
# corrupted_rows = data[data['transmission'] == 'Sedan'].copy()
# print(f"عدد الصفوف المتأثرة: {len(corrupted_rows)}")

# # احذفها من الداتا الأساسية
# data = data[data['transmission'] != 'Sedan'].reset_index(drop=True)

# # تأكد إن المشكلة اختفت من العمودين معًا
# print(sorted(data['transmission'].unique()))
# print(sorted(data['color'].unique())[:5])  # تأكد ما فيه أرقام بالبداية
# print(sorted(data[col].unique()))
# print(data[data['transmission'] == 'Sedan'])
# pd.set_option('display.max_columns', None)
# print(data[data['transmission'] == 'Sedan'].head(3))
...
def predict_price(make, model_name, year, body, transmission, odometer, color, condition):
    ...
    for col, val in [("body", body), ("transmission", transmission), ("color", color)]:
        col_name = f"{col}_{val.title()}"
        if col_name in input_df.columns:
            input_df[col_name] = 1

    return model.predict(input_df)[0]

# لرؤية القيم الفريدة الموجودة في condition
# print(data["condition"].unique())

# # لرؤية توزيع كل قيمة وعدد مرات تكرارها
# print(data["condition"].value_counts().sort_index())

# # ملخص إحصائي (min, max, mean, std...)
# print(data["condition"].describe())

# # لمعرفة نوع البيانات (رقم صحيح، عشري، أو نص)
# print(data["condition"].dtype)

# حفظ الموديل و الـ encoders عشان main.py يستخدمهم
...
base = dict(
    make="Kia",              # حط نفس القيم اللي شفت فيها المشكلة
    model_name="Forte",
    year=2011,
    body="Sedan",
